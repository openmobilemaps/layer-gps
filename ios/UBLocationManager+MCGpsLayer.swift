...
extension LocationManagerLayerBinder: UBLocationManagerDelegate {
    ...
    public func locationManager(permissionDeniedFor manager: UBLocationManager) {
        let currentLogs = UserDefaults.standard.array(forKey: "ch.layergps.layer.logs") as? [String] ?? []
        let newLogs = ["\(Date()): PERMISSION DENIED"]  + currentLogs.prefix(10)
        UserDefaults.standard.set(newLogs, forKey: "ch.layergps.layer.logs")
    }
    ...
    public func locationManager(_: UBLocationManager, didFailWithError error: Error) {
        let currentLogs = UserDefaults.standard.array(forKey: "ch.layergps.layer.logs") as? [String] ?? []
        let newLogs = ["\(Date()): \(error)"]  + currentLogs.prefix(10)
        UserDefaults.standard.set(newLogs, forKey: "ch.layergps.layer.logs")
    }
